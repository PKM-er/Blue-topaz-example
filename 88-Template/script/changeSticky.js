...
module.exports = async function colorclock (params) {
  ...
  if(pickedSticky == "1" && stickyContent){
    let newContent = fileContent.replace(stickyPatter1,  `<p class="stickies">\n${stickyContent}\n</p>\n`)
    await app.vault.adapter.fs.writeFileSync(filePath, newContent,"utf8")
  }else if(pickedSticky == "2" && stickyContent){
    let newContent = fileContent.replace(stickyPatter2, `<p class="stickies2" style="width: max(220px, 30%)" >\n${stickyContent}\n</p>\n`)
    await app.vault.adapter.fs.writeFileSync(filePath, newContent,"utf8")
  }else if(pickedSticky == "3" && stickyContent){
    let newContent = fileContent.replace(stickyPatter3, `<p class="stickies">\n${stickyContent}\n</p>\n`)
    await app.vault.adapter.fs.writeFileSync(filePath, newContent,"utf8")
  }else if(pickedSticky == "4" && stickyContent){
    let newContent = fileContent.replace(stickyPatter4, `<p class="stickies2" style="width: max(220px, 30%)" >\n${stickyContent}\n</p>\n`)
    await app.vault.adapter.fs.writeFileSync(filePath, newContent,"utf8")
  }else{
