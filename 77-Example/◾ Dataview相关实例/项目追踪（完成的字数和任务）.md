...
/////
dv.el("dvjs", "");
    dv.el("bold", "按文件夹检索:&ensp;", {
        attr: { "style": "font-size:1.2em;display: inline-table" },
    });
    dv.el("bold", searchFolerDropdownMaker('sourceFolder', dv.current().file.path), {
       attr: { "style":"display: inline-table" },
    });
	 dv.el("bold", "按标签检索:&ensp;", {
	        attr: { "style": "font-size:1.2em;display: inline-table" },
	    });
	dv.el("bold", tagsDropdownMaker('sourceTag', dv.current().file.path), {
       attr: { "style":"display: inline-table" },
    });
    dv.el("br")
	 dv.el("bold", "排除的文件夹:&ensp;", {
	        attr: { "style": "font-size:1.2em;display: inline-table" },
	    });
	dv.el("bold", exFolerDropdownMaker('exFolder', dv.current().file.path), {
       attr: { "style":"display: inline-table" },
    });
	 dv.el("bold", "排除的标签:&ensp;", {
	        attr: { "style": "font-size:1.2em;display: inline-table" },
	    });
	dv.el("bold", extagsDropdownMaker('excludeTag', dv.current().file.path), {
       attr: { "style":"display: inline-table" },
    });
    dv.el("br")
     dv.el("bold", "按文件名过滤:&ensp;", {
	        attr: { "style": "font-size:1.2em;display: inline-table" },
	    });
	dv.el("bold", inputMaker('searchText', dv.current().file.path), {
       attr: { "style":"display: inline-table" },
    });
