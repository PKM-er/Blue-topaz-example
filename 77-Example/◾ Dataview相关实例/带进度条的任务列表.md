...
-  百分比任务条通过metaedit插件动态更新
  `="<progress  class='progress' value=" + this.Completed +" max="+ this.Total+"></progress> "+this.Completed+"/"+ this.Total`
-  百分比任务条通过dv插件动态实时更新
  `$= const value = ((dv.current().file.tasks.where(t => t.completed).length) / (dv.current().file.tasks).length || 0) * 100; "<progress value='" + value.toFixed(2) + "' max='100'></progress>"`
	- [x] 第一个任务
	- [ ] 第二个任务
	- [x] 第三个任务 ✅ 2022-06-06
		- [ ] 第三个子任务
